...
1. **계측 실패는 테스트 실패가 아니다.** kube-slint가 메트릭을 스크랩할 수 없으면 해당 결과는 미계측으로 기록된다. E2E 테스트는 계속 진행된다.
2. **정책 위반은 CI를 실패시킬 수 있다.** 임계치 미달 또는 기준선 대비 회귀가 감지되면 게이트 스텝이 비정상 종료 코드로 종료되고 CI 작업이 실패한다.
3. **가드레일 평가와 correctness 테스트는 분리된다.** 기존 E2E 단언과 kube-slint 게이트 결과는 독립적인 신호이며, 둘 다 독립적으로 실패할 수 있다.
...
`slint.DefaultSpecs()`(구 이름: `DefaultV3Specs`, `BaselineV3Specs`)는 kubebuilder로 생성된 오퍼레이터를 위해 설계된 프리셋 스펙 세트를 반환한다. 포함 항목은 다음과 같다:
...
sess := slint.NewSession(slint.SessionConfig{
    ...
    // bearer token은 kubectl args에 넣지 않고 pod 내부 mounted token을 읽음.
    ServiceAccountName: "kube-slint-scraper",
...
**RBAC 주의사항:** 하네스는 curl 기반 페처를 사용하여 메트릭 엔드포인트를 스크랩하기 위한 임시 파드를 생성한다. `slint-gate init --emit-rbac rbac.yaml`은 대상 네임스페이스 안에 필요한 ServiceAccount, Role, RoleBinding을 생성하는 매니페스트를 출력한다.

`slint-gate init --profile kubebuilder-operator`는 기존 `init`의 하위호환 확장이다(현재 지원되는 유일한 profile). `--profile`을 생략하면 기존 `init` 출력이 그대로 유지된다.

**출력:** `sess.End(ctx)` 호출 시 두 파일이 작성된다:
...
기본 curl pod 경로는 pod 내부의 mounted ServiceAccount token을 직접 읽는다. 따라서 bearer token이 `kubectl` 인자나 Pod command에 직접 들어가지 않는다. `pkg/slint`의 token 읽기 헬퍼는 호환용 API로 유지된다:
...
새 코드에서는 이 값을 curl pod command에 삽입하지 않는다.
...
기본값은 `slint.ServiceURLHTTPS` (`"https://%s.%s.svc:8443/metrics"`)다.

기본적으로 `ServiceURLFormat`은 클러스터 내부 Service 주소
(`<service>.<namespace>.svc` 또는 `.svc.cluster.local`, `http`/`https`만
허용)로만 해석되어야 한다 — 그 외에는 curl pod 생성 전에 거부되므로
잘못 설정되거나 악의적인 외부 URL이 스크랩용 Authorization 토큰을 받을 수
없다. 이 기본값을 명시적으로 해제하려면 `DangerouslyAllowExternalMetricsURL: true`를 설정한다.

`TLSInsecureSkipVerify`는 `DangerouslySkipTLSVerify`로 대체된 deprecated
필드다(효과는 동일하고 이름만 명확함) — 자체 서명 인증서를 쓰는 개발
클러스터와의 호환을 위해 남아 있지만 TLS 검증을 약화한다. 공유 CI나
운영과 유사한 환경에서는 위험을 명시적으로 수용한 경우가 아니면 켜지
않는다. 기본값은 `false`다.
...
게이트 CLI는 `cmd/slint-gate`에 위치한 Go 바이너리다. 이전 Python prototype(`hack/slint_gate.py`)은 완전히 제거되었으며, 운영 경로는 Go 바이너리만 사용한다.
...
| `--exit-on` | `NEVER` | 종료 코드 1을 반환할 gate_result 조건 (아래 참조). `--fail-on`은 하위호환용 deprecated alias로 계속 동작한다. |
...
**종료 동작:** 기본값(`NEVER`)에서는 항상 0으로 종료한다. `--exit-on FAIL` 이상을 지정하면 해당 조건에서 exit 1이 반환된다. 알 수 없는 `--exit-on` 값은 즉시 거부된다. `--fail-on`을 직접 지정해도 여전히 동작하지만, `--exit-on`을 쓰라는 안내가 stderr에 출력된다(`--exit-on`과 동시에 지정하면 `--exit-on`이 우선).
...
`policy.promote_to_fail`과 CLI `--exit-on`은 별개의 제어 레이어다. 이전에는 각각 `fail_on`/`--fail-on`으로 불려서 같은 개념처럼 보였지만, 실제로는 서로 다른 레이어다 — 이름이 헷갈렸던 부분을 정리한 것이다. 기존 `fail_on`/`--fail-on`은 deprecated alias로 계속 동작한다 (`docs/sli-gate-onboarding-ux.md`의 네이밍 절 참고).
...
`observed`는 일반적으로 숫자다. 다만 baseline=0이고 current가 0이 아닌 regression처럼 수치 비율로 표현하기 어려운 경우에는 `"baseline_zero_current_nonzero"` 같은 문자열 marker가 들어갈 수 있다. jq 스크립트나 대시보드는 `observed`가 항상 숫자라고 가정하면 안 된다.

---

### 4. 관측성 스택 배포 (Kustomize)

kube-slint는 클러스터에 메트릭 수집 인프라를 설치하는 Kustomize 베이스를 제공한다.
...
규칙:
- 항상 태그 또는 SHA로 고정한다. `?ref=main`은 절대 사용하지 않는다.
- 오버레이에 반드시 `namespace:`를 선언한다. 베이스는 네임스페이스를 가정하지 않는다.

**ServiceMonitor 레이블**

베이스의 ServiceMonitor에는 레이블이 하드코딩되어 있다. Prometheus 오퍼레이터의 셀렉터에 맞게 전략적 머지 패치로 이를 재정의해야 한다.
...
전체 온보딩 튜토리얼은 `test/consumer-onboarding/kustomize-remote-consumer/`에서 확인할 수 있다.

---

## 계측 모드

kube-slint는 세션 또는 스펙별로 설정할 수 있는 세 가지 1급 계측 모드를 지원한다:

| 모드 | 설명 |
|---|---|
| `InsideSnapshot` (기본) | 세션 시작과 종료 시 스냅샷 기반 수집; 두 값의 차이로 델타를 계산 |
| `InsideAnnotation` | **예약됨, 아직 미구현** — 현재는 `InsideSnapshot`과 동일하게 동작한다(기록되는 run-mode 라벨만 다름); 어노테이션 경계 정밀도가 필요한 용도로는 아직 사용하지 말 것 |
...
게이트 모델 구성 요소는 모두 완성되었다.

**임계치 검사** (완료): `sli-summary.json`의 각 메트릭 결과가 `policy.yaml`의 임계치 규칙과 비교된다. `promote_to_fail`에 `threshold_miss`가 포함된 경우 임계치 미달 시 `gate_result`가 `FAIL`로 설정된다.

**회귀 감지** (완료): `--baseline`이 제공된 경우, 각 메트릭 결과가 저장된 기준선 값과 비교된다. 변화량이 `tolerance_percent`를 초과하면 회귀로 플래그된다. `promote_to_fail`에 `regression_detected`가 포함된 경우 회귀 감지 시 `gate_result`가 `FAIL`로 설정된다.

**Coverage governance** (완료): `coverage.required: true`이면 측정된 모든 scalar SLI가 threshold rule로 gate되거나 `coverage.informational`에 명시돼야 한다. `coverage_gap`이 `promote_to_fail`에 있으면 coverage gap은 `gate_result=FAIL`이 되며, 생성되는 policy에는 기본 포함된다. 이 검사를 끄려면 `coverage.required: false`로 둔다.

---

## 보안 기본값

kube-slint의 기본 계측 경로는 네임스페이스 스코프다:

- 생성 RBAC는 ServiceAccount, Role, RoleBinding을 사용한다.
- 기본 경로에는 ClusterRoleBinding이 필요하지 않다.
- curl pod는 pod 내부에 mount된 ServiceAccount token을 읽는다.
- command/error 출력은 일반적인 token/secret 형태를 redaction한다.
- 계측이 불충분한 경우 `NO_GRADE`가 1급 gate 결과로 노출된다.
- `ServiceURLFormat`은 curl pod 생성 전에 검증된다 — 외부 host, 지원하지
  않는 scheme, 잘못된 형식의 service/namespace 값은 기본적으로 거부된다
  (해제하려면 `DangerouslyAllowExternalMetricsURL` 참고).
- `kube-system`/`kube-public`/`kube-node-lease`는 기본적으로 계측 대상
  namespace로 거부된다 (해제하려면 `DangerouslyAllowKubeSystemNamespace`
  참고).
...
워크플로우에 추가할 수 있다. 현재 action은 Go CLI를 source에서 실행하므로
먼저 Go를 설정해야 한다:
...
| `exit-on` | `` (`fail-on`으로 폴백) | `NEVER`\|`FAIL`\|`FAIL_OR_WARN`\|`FAIL_OR_NOGRADE`\|`FAIL_WARN_OR_NOGRADE`. `fail-on`보다 우선 적용됨 (`fail-on`은 deprecated, 기본값 `FAIL_OR_NOGRADE`로 계속 동작). |
...
장기 CI에서는 가능하면 `main` 대신 tag 또는 SHA에 pin한다.
...
외부 클러스터 없이 kind 로컬 클러스터로 전체 흐름을 체험할 수 있는 예제가 `examples/kind-hello-operator/`에 포함되어 있다.
...
`setup.sh`는 kind 클러스터 생성만 수행한다. Docker 이미지 빌드, kind load, kubectl apply 등은 스크립트 실행 후 출력되는 Next steps 안내를 따른다. 상세 안내는 `examples/kind-hello-operator/README.md`를 참조한다.

---

## 기준선 관리

기준선은 회귀 비교를 위해 이전 게이트의 메트릭 결과를 저장한다.
...
**승인 요구사항:** 기준선 변경은 일반 기능 PR이 아닌 별도의 승인 PR에 포함되어야 한다. 이는 성능이 저하된 실행 결과가 자동으로 회귀 기준점을 초기화하는 것을 방지한다.

**기준선 없는 첫 실행:** `--baseline`을 지정하지 않으면 회귀 감지를 건너뛰고 `gate_result`가 `WARN`(비차단)으로 설정된다. 이는 온보딩 후 첫 실행에서 예상되는 동작이다.
...
Apache License, Version 2.0에 따라 배포된다.
자세한 내용은 http://www.apache.org/licenses/LICENSE-2.0 를 참조한다.

본 소프트웨어는 관련 법령이나 서면 동의에 의해 별도로 명시되지 않는 한, 명시적 또는 묵시적 어떠한 종류의 보증도 없이 "있는 그대로" 배포된다.
