...
`slint-gate init --profile kubebuilder-operator`는 기존 `init`의 하위호환 확장입니다(현재 지원되는 유일한 profile). `--profile`을 생략하면 기존 `init` 출력이 그대로 유지됩니다.
...
| `--exit-on` | `NEVER` | 종료 코드 1을 반환할 gate_result 조건 (아래 참조). `--fail-on`은 하위호환용 deprecated alias로 계속 동작합니다. |
...
**종료 동작:** 기본값(`NEVER`)에서는 항상 0으로 종료합니다. `--exit-on FAIL` 이상을 지정하면 해당 조건에서 exit 1이 반환됩니다. 알 수 없는 `--exit-on` 값은 즉시 거부됩니다. `--fail-on`을 직접 지정하면 여전히 동작하지만, `--exit-on`을 쓰라는 안내가 stderr에 출력됩니다(`--exit-on`과 동시에 지정하면 `--exit-on`이 우선).
...
`policy.promote_to_fail`과 CLI `--exit-on`은 별개의 제어 레이어입니다. 이전에는 각각 `fail_on`/`--fail-on`으로 불려서 같은 개념처럼 보였지만, 실제로는 서로 다른 레이어입니다 — 이름이 헷갈렸던 부분을 이번에 정리했습니다. 기존 `fail_on`/`--fail-on`은 deprecated alias로 계속 동작합니다 (`docs/sli-gate-onboarding-ux.md`의 네이밍 절 참고).

| 레이어 | 역할 |
|---|---|
| `policy.promote_to_fail` (구 `fail_on`) | 어떤 위반 항목을 `gate_result=FAIL`로 승격할지 결정. 목록에 없는 위반은 `WARN`이 됩니다. 실패한 검사가 `PASS`가 되는 일은 없습니다. |
| CLI `--exit-on` (구 `--fail-on`) | 어떤 `gate_result`에서 프로세스를 exit 1로 종료할지 결정 |

`promote_to_fail`을 생략하거나 빈 배열로 두면 기본 hard-fail 항목인 `threshold_miss`, `regression_detected`가 적용됩니다. `fail_on`을 사용하면 `slint-gate-summary.json`의 `policy_warnings`에 마이그레이션 안내가 추가됩니다.
...
**임계치 검사** (완료): `sli-summary.json`의 각 메트릭 결과가 `policy.yaml`의 임계치 규칙과 비교됩니다. `promote_to_fail`에 `threshold_miss`가 포함된 경우 임계치 미달 시 `gate_result`가 `FAIL`로 설정됩니다.

**회귀 감지** (완료): `--baseline`이 제공된 경우, 각 메트릭 결과가 저장된 기준선 값과 비교됩니다. 변화량이 `tolerance_percent`를 초과하면 회귀로 플래그됩니다. `promote_to_fail`에 `regression_detected`가 포함된 경우 회귀 감지 시 `gate_result`가 `FAIL`로 설정됩니다.
