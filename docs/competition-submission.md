...
**최신 릴리즈**: v1.7.0 (source-neutral/window SLI hardening: JSON/expvar `SnapshotFetcher`, `WindowFetcher`/`promrange`, scalar window modes including `window_p95`/`window_ratio`, strict coverage governance with `coverage.required`/`coverage.informational`/`coverage_gap`, source-selection and window SLI docs, plus stale-doc and strict-default consistency fixes. v1.6.0 included `pkg/gate` 분리, `baseline merge --mode review-existing/force-replace`, interactive `slint-gate wizard`, F4 quoted label parser 수정, and 16 confirmed pre-release adversarial-review fixes.)
