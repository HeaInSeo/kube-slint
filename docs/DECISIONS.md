...
- Rationale:
  - This directly serves task 4 (baseline merge `review-existing` mode, D-023) too: that mode needs the same operator-direction inference `baseline_diff.go` already had, and reusing `gate.LowerIsBetter`/`gate.HigherIsBetter` avoids creating a *third* copy of it instead of consolidating the two that already existed.
  ...
  - This also sets up the baseline merge `review-existing` mode (next backlog item): that mode needs the same operator-direction inference `baseline_diff.go` already had, and reusing `gate.LowerIsBetter`/`gate.HigherIsBetter` avoids creating a *third* copy of it instead of consolidating the two that already existed.
